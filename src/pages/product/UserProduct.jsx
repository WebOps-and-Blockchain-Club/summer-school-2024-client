...
const UserProduct = (props) => {
    ...
    const del =(prod)=>{
        async function deleteProduct() {
            ...
            const res = await axios.delete(`${process.env.REACT_APP_BACKEND_BASE_URL}/product/${prod.productId}`,
              {
                headers: {
    ...
    useEffect(() => {
        async function fetchProducts() {
            const token = localStorage.getItem("token");
            const res = await axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/products/user`,
              {
                headers: {
                  Authorization: `Bearer ${token}`,
                },
  ...
  <div className="w-full max-w-7xl px-4 md:px-5 lg-6 mx-auto">
    ...
    {products&&products.map((product, index) => (<div className="rounded-3xl border-2 border-gray-200 p-4 lg:p-8 grid grid-cols-12 mb-8 max-lg:max-w-lg max-lg:mx-auto gap-y-4 dark:bg-black ">
